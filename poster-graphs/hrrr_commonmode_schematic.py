...
# --- EDIT TITLE HERE ---
TITLE = "Why Reanalysis Wind Is Common-Mode at Sensor Scale"
...
x0, y0 = 3.0, 3.0
ax.add_patch(Rectangle((x0, y0), cell, cell, fill=True,
                       facecolor="#f7e3e3", edgecolor=GRIDLN, lw=2.5, zorder=1))
ax.text(x0 + cell/2, y0 + cell + 0.12, "one 3 km HRRR grid cell",
        ha="center", va="bottom", fontsize=FS, color=GRIDLN)

# --- wind field EVERYWHERE: identical vector at every grid node ---
wx, wy = 0.75, 0.38
for gx in np.arange(0.6, 9.0, 1.0):
    for gy in np.arange(0.6, 9.0, 1.0):
        ax.add_patch(FancyArrow(gx, gy, wx, wy, width=0.02,
                                head_width=0.13, head_length=0.12,
                                length_includes_head=True, color="#b9c9e6",
                                zorder=2))

# ~10 sensor dots spread across several cells (1-3 km apart)
rng = np.random.default_rng(7)
sensors = np.array([
    [3.5, 3.6], [4.4, 3.9], [5.1, 3.4], [3.8, 4.8], [4.9, 4.9],   # in center cell
    [5.4, 4.3], [4.2, 5.4], [2.2, 2.4], [6.6, 5.8], [2.6, 6.3],   # neighbours
])
ax.scatter(sensors[:, 0], sensors[:, 1], s=150, color=BLUE,
           edgecolor="white", linewidth=1.8, zorder=5)

# emphasize the wind AT each sensor (same vector -> common-mode)
for sx, sy in sensors:
    ax.add_patch(FancyArrow(sx, sy, wx, wy, width=0.03,
                            head_width=0.17, head_length=0.15,
                            length_includes_head=True, color=WIND,
                            zorder=4))
...
# scale reference
ax.annotate("", xy=(3.5, 2.55), xytext=(4.4, 2.55),
            arrowprops=dict(arrowstyle="<->", lw=1.2, color=SECOND))
ax.text(3.95, 2.35, "~1–3 km sensor spacing", ha="center", va="top",
        fontsize=FS, color=SECOND)
