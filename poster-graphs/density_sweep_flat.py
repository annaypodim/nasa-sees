...
# --- EDIT TITLE HERE ---
TITLE = "On Flat Terrain, No Density Level Lets the Model Beat IDW"
...
ax.fill_between(N, idw, upper, where=(upper > np.array(idw)),
                interpolate=True, color="#e34948", alpha=0.12, zorder=1)

ax.plot(N, idw,  color=BLUE,   marker="o", ms=7, lw=2, label="IDW baseline", zorder=4)
ax.plot(N, krig, color=AQUA,   marker="s", ms=7, lw=2, label="GNN + kriging prior", zorder=3)
ax.plot(N, idwp, color=ORANGE, marker="^", ms=7, lw=2, label="GNN + IDW prior", zorder=3)

ax.text(9, 5.55, "GNN loses here", color="#b02b2b", fontsize=FS,
        ha="left", va="bottom")
