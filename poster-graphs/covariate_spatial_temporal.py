...
TITLE = "Spatial v. Temporal Variation in Wind Speed, Temperature, AOD and Elevation"
SUBTITLE = ""
# -----------------------------------------------------------------------------
import glob
import re
...
a = pd.read_csv(
    PGH / "aod" / "aod_points_2023-01-01_2023-12-31.csv", parse_dates=["datetime"]
)
a["day"] = a["datetime"].dt.floor("D")
aod = a.pivot_table(
    index="day", columns="id", values="aod_055", aggfunc="mean"
).to_numpy()
...
elev_z = (elev_m - elev_m.mean()) / (
    elev_m.std() + 1e-12
)  # spatial std = 1, temporal = 0
...
for name, (tstd, sstd, color) in points.items():
    ax.scatter(
        tstd, sstd, s=180, color=color, edgecolor="white", linewidth=1.5, zorder=4
    )
    # place each label clear of its point and of neighbours
    off = {
        "Elevation": (0.04, -0.05, "left"),
        "Wind speed": (0.03, 0.04, "left"),
        "AOD": (-0.03, -0.05, "right"),
        "Temperature": (-0.03, 0.05, "right"),
    }[name]
    ax.annotate(
        name,
        (tstd, sstd),
        xytext=(tstd + off[0], sstd + off[1]),
        fontsize=FS,
        color=color,
        va="center",
        ha=off[2],
...
ax.set_xlabel("Temporal variability", fontsize=FS)
ax.set_ylabel("Spatial variability", fontsize=FS)
...
for name, (t, s, _) in points.items():
    print(
        f"  {name:12s} temporal_std={t:.3f}  spatial_std={s:.3f}  ratio(sp/tp)²={(s / t) ** 2 if t > 0 else float('inf'):.3f}"
    )
